...
class App {
    ...
    updateUI(): void {
        const returnBook = (book: Book) => {
            const user = this._service.getUsers().find(u => u.getId === book.getBorrower) as User;
            ...
            if(user) {
                ...
                this.showPonModal(`${book.getName} by ${book.getAuthor} (${book.getReleaseYear}) успішно повернена користувачем ${user.getId} ${user.getName} (${user.getEmail}).`);
                this.updateUI();
            }
        }
        ...
        const borrowBook = (book: Book) => {
            const handleSubmit = (e: Event) => {
                ...
                if(this._validation.validateForm(form)) {
                    ...
                    const user = this._service.getUsers().find(u => u.getId === userId) as User;
                    Object.setPrototypeOf(book, Book.prototype);
                    Object.setPrototypeOf(user, User.prototype);
                    ...
                    if(user) {
                        try {
                            ...
                            this._service.updateBook(book.getId, book);
                            this._service.updateUser(user.getId, user);
                            this.showPonModal(`${book.getName} by ${book.getAuthor} (${book.getReleaseYear}) успішно позичена користувачем ${user.getId} ${user.getName} (${user.getEmail}).`);
                            this.updateUI();
                        }
                        catch (error) {
                            this.showPonModal(`Не вдалося позичити книгу: ${(error as Error).message}`);
                        }
                    }
                }
        }
        ...
        this._service.getBooks().forEach(book => {
            ...
            bookItem.className = 'd-flex justify-content-between align-items-center border-bottom py-3';

            const bookInfo = document.createElement('span');
            bookInfo.textContent = `${book.getName} by ${book.getAuthor} (${book.getReleaseYear})`;

            const actionButton = document.createElement('button');
            actionButton.className = book.isBorrowed() ? 'btn btn-warning' : 'btn btn-primary';
